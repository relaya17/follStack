...
type RoomMessage =
  | { type: 'avatar_joined'; avatar: Avatar }
  | { type: 'avatar_left'; avatarId: string }
  | { type: 'avatar_moved'; avatarId: string; position: Avatar['position']; rotation: Avatar['rotation'] }
  | { type: 'avatar_updated'; avatarId: string; updates: Partial<Avatar> }
  | { type: 'chat_message'; message: ChatMessage }
  | { type: 'hand_raised'; avatarId: string; isHandRaised: boolean }
...
export function MetaverseClassroom({ isOpen, onClose, roomId }: MetaverseClassroomProps) {
  ...
  const handleRoomMessage = (data: RoomMessage) => {
    switch (data.type) {
      case 'avatar_joined':
        setAvatars((prev: Avatar[]) => [...prev, data.avatar])
        break
      case 'avatar_left':
        setAvatars((prev: Avatar[]) => prev.filter((avatar: Avatar) => avatar.id !== data.avatarId))
        break
      case 'avatar_moved':
        setAvatars((prev: Avatar[]) => prev.map((avatar: Avatar) => 
  ...
  const moveAvatar = (direction: 'forward' | 'backward' | 'left' | 'right') => {
    ...
    setCurrentUser((prev: Avatar | null) => prev ? { ...prev, position: newPosition } : null)
    setAvatars((prev: Avatar[]) => prev.map((avatar: Avatar) => 
