  ...
  const initializeARLibrary = async () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      arContextRef.current = {
        ...
        addObject: (obj: ARObject) => setArObjects((prev: ARObject[]) => [...prev, obj]),
        removeObject: (id: string) =>
          setArObjects((previousObjects: ARObject[]) =>
            previousObjects.filter((arObject: ARObject) => arObject.id !== id)
          )
      }
    }
  }
...
console.log(greet('World'));`
  ...
  const handleObjectInteraction = (objectId: string) => {
    const object = arObjects.find((obj: ARObject) => obj.id === objectId)
    if (object) {
        ...
        return (
          <div className="bg-white p-4 rounded-lg shadow-lg">
            <svg width="200" height="200" viewBox="0 0 200 200">
              {object.content.nodes.map((node: { id: string; label: string; x: number; y: number }) => (
                <circle
                  key={node.id}
                  cx={node.x + 100}
                  cy={node.y + 100}
              ...
              {object.content.edges.map((edge: { from: string; to: string }, index: number) => (
                <line
                  key={index}
                  x1="100"
                  y1="100"
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 rtl:space-x-reverse">
              ...
              <button
                ...
                className="p-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                aria-label="איפוס אובייקטים"
                title="איפוס אובייקטים"
              >
